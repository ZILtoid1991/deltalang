...
public import deltalang.jsvar;
public import deltalang.common;
import deltalang.expressions;
...
+/
class NonScriptCatchableException : DSException {
...
class ScriptCompileException : DSException {
	string s;
	int lineNumber;
...
class ScriptRuntimeException : DSException {
	string s;
	int lineNumber;
	this(string msg, string s, int lineNumber, string file = __FILE__, size_t line = __LINE__) {
		this.s = s;
...
class ScriptException : DSException {
	///
	var payload;
	///
	ScriptLocation loc;
...
private enum string[] keywords = [
	...
	"var", "for", "try", "new",
	"if", "do",
	//keywords for type safety through static typing (similar to typescript)
					...
					foreach(idx, dchar ch; text[started .. pos]) {
						if(escaped) {
							...
							switch(ch) {
								...
								case '\'': copy ~= "'"; break;
								default:
									throw new ScriptCompileException("Unknown escape char " ~ cast(char) ch, token.scriptFilename, 
											token.lineNumber);
							}
							continue;
						} else if(ch == '\\') {
...
void addChildElementsOfExpressionToScriptExpressionObject(ClassInfo c, Expression _thisin, PrototypeObject sc, 
		ref var obj) {
	...
	static if(__traits(compiles, __traits(getMember, mixin(__MODULE__), itemName))) {
		...
		static if(is(Class : Expression)) if(c == typeid(Class)) {
			...
			foreach(memberName; __traits(allMembers, Class)) {
				...
				static if(is(typeof(member) : string) || is(typeof(member) : long) || is(typeof(member) : real) || 
...
ScriptToken requireNextToken(MyTokenStreamHere)(ref MyTokenStreamHere tokens, ScriptToken.Type type, string str = null, 
	...
	if(next.type != type || (str !is null && next.str != str))
		throw new ScriptCompileException("unexpected '"~next.str~"' while expecting " ~ to!string(type) ~ " " ~ str, 
				next.scriptFilename, next.lineNumber, file, line);
...
Expression parsePart(MyTokenStreamHere)(ref MyTokenStreamHere tokens) {
	if(!tokens.empty) {
		...
		else if(token.type == ScriptToken.Type.symbol && (token.str == "-" || token.str == "+" || token.str == "!" || 
				token.str == "~")) {
			auto op = token.str;
			tokens.popFront();
			...
			if(token.type == ScriptToken.Type.int_number)
				...
				switch(token.str) {
					...
					case "[":
						...
						moreElements:
						if(tokens.empty)
							throw new ScriptCompileException("unexpected end of file when reading array literal", token.scriptFilename, 
									token.lineNumber);
					...
					case "#{":
						...
						*/
						...
						if(tokens.empty)
							throw new ScriptCompileException("unexpected end of file when reading object literal", token.scriptFilename, 
									token.lineNumber);
						...
						if(key.type != ScriptToken.Type.string && key.type != ScriptToken.Type.identifier) {
							throw new ScriptCompileException("unexpected '"~key.str~"' when reading object literal", key.scriptFilename, 
									key.lineNumber);
				...
				unknown:
				throw new ScriptCompileException("unexpected '"~token.str~"' when reading ident", token.scriptFilename, 
						token.lineNumber);
			}
		}
...
Expression parseArguments(MyTokenStreamHere)(ref MyTokenStreamHere tokens, Expression exp, ref Expression[] where) {
	...
	if(tokens.empty)
		throw new ScriptCompileException("unexpected end of file when parsing call expression", peek.scriptFilename, 
				peek.lineNumber);
	peek = tokens.front;
	if(peek.type == ScriptToken.Type.symbol && peek.str == ",") {
		tokens.popFront();
		...
		throw new ScriptCompileException("unexpected '"~peek.str~"' when reading argument list", peek.scriptFilename, 
				peek.lineNumber);
	...
	loop: while(!tokens.empty) {
		...
		if(peek.type == ScriptToken.Type.symbol) {
			switch(peek.str) {
				...
				default:
					throw new ScriptCompileException("Parse error, unexpected " ~ peek.str ~ " when looking for operator", 
							peek.scriptFilename, peek.lineNumber);
			}
...
Expression parseExpression(MyTokenStreamHere)(ref MyTokenStreamHere tokens, bool consumeEnd = false) {
	...
	if(!tokens.empty) {
		...
		if(tokens.peekNextToken(ScriptToken.Type.symbol, "{")) {
			...
			switch(ident.str) {
				...
				default:
					throw new ScriptCompileException("unexpected " ~ ident.str ~ ". valid scope(idents) are success, failure, and exit"
							, ident.scriptFilename, ident.lineNumber);
			}
			...
			*/
			...
			vars.initializers = [new ObjectLiteralExpression(staticScopeBacking), 
					new ObjectLiteralExpression(instanceScopeBacking)];
			expressions ~= vars;
			...
			while(!tokens.peekNextToken(ScriptToken.Type.symbol, "}")) {
				...
				} else throw new ScriptCompileException("Unexpected " ~ tokens.front.str ~ " when reading class decl", 
						tokens.front.scriptFilename, tokens.front.lineNumber);
			}
			...
			while(!tokens.peekNextToken(ScriptToken.Type.symbol, "}")) {
				...
				if(tokens.peekNextToken(ScriptToken.Type.keyword, "case")) {
					...
					while(!tokens.peekNextToken(ScriptToken.Type.keyword, "default") && !tokens.peekNextToken(ScriptToken.Type.keyword,
							"case") && !tokens.peekNextToken(ScriptToken.Type.symbol, "}")) {
					...
					e.cases ~= c;
					e.default_ = c;
				} else throw new ScriptCompileException("A switch statement must consists of cases and a default, nothing else ", 
						tokens.front.scriptFilename, tokens.front.lineNumber);
			}
			...
			while(tokens.peekNextToken(ScriptToken.Type.keyword, "catch")) {
				if(hadFinally)
					throw new ScriptCompileException("Catch must come before finally", tokens.front.scriptFilename, 
				...
				auto ident = tokens.requireNextToken(ScriptToken.Type.identifier);
				if(tokens.empty) throw new ScriptCompileException("Catch specifier not closed", ident.scriptFilename, 
						ident.lineNumber);
				auto next = tokens.front;
				if(next.type == ScriptToken.Type.identifier) {
					...
					if(next.type != ScriptToken.Type.symbol || next.str != ")")
						throw new ScriptCompileException("ss Unexpected " ~ next.str ~ " when expecting ')'", next.scriptFilename, 
								next.lineNumber);
					tokens.popFront();
				}
	...
	if(expectedEnd.length && tokens.empty && consumeEnd) // going loose on final ; at the end of input for repl convenience
		throw new ScriptCompileException("Parse error, unexpected end of input when reading expression, expecting " ~ 
				expectedEnd, first.scriptFilename, first.lineNumber);
...
VariableDeclaration parseVariableDeclaration(MyTokenStreamHere)(ref MyTokenStreamHere tokens, string termination) {
	...
	equalOk= true;
	if(tokens.empty)
		throw new ScriptCompileException("Parse error, dangling var at end of file", firstToken.scriptFilename, 
	...
	tokens.popFront;
	if(tokens.empty)
		throw new ScriptCompileException("Parse error, incomplete var declaration at end of file", firstToken.scriptFilename, 
				firstToken.lineNumber);
	...
	if(identifier.type != ScriptToken.Type.identifier)
		throw new ScriptCompileException("Parse error, found '"~identifier.str~"' when expecting var identifier", 
				identifier.scriptFilename, identifier.lineNumber);

	//tokens.popFront();

	tryTermination:
	if(tokens.empty)
		throw new ScriptCompileException("Parse error, missing ; after var declaration at end of file", 
				firstToken.scriptFilename, firstToken.lineNumber);
	...
	if(peek.type == ScriptToken.Type.symbol) {
		if(peek.str == "=") {
			...
			throw new ScriptCompileException("Parse error, unexpected '"~peek.str~"' when reading var declaration symbol", 
					peek.scriptFilename, peek.lineNumber);
	} else
		throw new ScriptCompileException("Parse error, unexpected non-symbol '"~peek.str~"' when reading var declaration", 
				peek.scriptFilename, peek.lineNumber);
...
Expression parseStatement(MyTokenStreamHere)(ref MyTokenStreamHere tokens, string terminatingSymbol = null) {
	...
	if(terminatingSymbol !is null && (tokens.front.type == ScriptToken.Type.symbol && tokens.front.str == 
			terminatingSymbol))
...
struct CompoundStatementRange(MyTokenStreamHere) {
	...
	void popFront() {
		while(!tokens.empty && (terminatingSymbol is null || !(tokens.front.type == ScriptToken.Type.symbol && 
				tokens.front.str == terminatingSymbol))) {
		...
		if(tokens.empty && terminatingSymbol !is null) {
			throw new ScriptCompileException("Reached end of file while trying to reach matching " ~ terminatingSymbol, null, 
					startingLine);
		}
...
//Expression[]
parseCompoundStatement(MyTokenStreamHere)(ref MyTokenStreamHere tokens, int startingLine = 1, 
		string terminatingSymbol = null) {
...
var interpretStream(MyTokenStreamHere)(MyTokenStreamHere tokens, var variables) 
	...
	return interpretStream(tokens,
		(variables.payloadType() == var.Type.Object && variables._payload._object !is null) ? variables._payload._object : 
				new PrototypeObject());
}
...
+/
var interpret(string code, var variables = null, string scriptFilename = null, string file = __FILE__, 
...
var interpretFile(File file, var globals) {
	...
	return interpretStream(lexScript(file.byLine.map!((a) => a.idup), file.name),
		(globals.payloadType() == var.Type.Object && globals._payload._object !is null) ? globals._payload._object : 
				new PrototypeObject());
}
...
void repl(bool enhanced = false)(var globals) {
	...
	import std.algorithm;
	auto variables = (globals.payloadType() == var.Type.Object && globals._payload._object !is null) ? 
			globals._payload._object : new PrototypeObject();
