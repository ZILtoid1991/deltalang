...
+/
module deltalang.jsvar;

import deltalang.common;
...
struct var {
	...
	*/
	...
	private enum Flags {
		TypeRestricted	=	1<<0,	///Restricts data to initial type
	}

	private uint flags;		///Stores info about an instance of var
	///Returns true if type is restricted to the initial one.
	public bool isTypeRestricted() @nogc @safe pure nothrow const {
		return flags & Flags.TypeRestricted;
	}
...
class PrototypeObject {
	...
	ref var _getMember(string name, bool recurse, bool throwOnFailure, string file = __FILE__, size_t line = __LINE__) {
		...
		if(throwOnFailure)
			throw new MissingMemberException("no such property " ~ name, file, line);
		var n;
		this._properties[name] = n;
		return this._properties[name];
	}

	ref var _setMember(string name, var t, bool recurse, bool throwOnFailure, bool suppressOverloading, string 
...
class DynamicTypeException : DSException {
